      ...
      <div class="id-card glass-card fade-up" :style="cardAccentVars">
        ...
        <div class="details-section">
          ...
          <div class="details-grid">
            <DetailItem icon="hash"     :label="`${entityLabel} ID`" :value="data.student.studentId" mono />
            <DetailItem v-if="data.student.class" icon="graduate" label="Class" :value="data.student.class" />
            <DetailItem
              v-for="f in dynamicDetailFields"
              :key="f.key"
              icon="badge"
              :label="f.label"
              :value="f.value"
...
// entityLabel comes from the project's metadata (e.g. "Patient", "Employee",
// "Attendee"). Falls back to "Student" so pre-existing education projects
// that were published before this field existed render identically to before.
const entityLabel = computed(() => data.value?.school?.entityLabel || 'Student')
...
const orgIcon = computed(() => INDUSTRY_ICONS[data.value?.school?.industry] || 'school')

// Field definitions marked showOnCard render here, using the value stored in
// student.data[key]. Fields without a stored value are skipped so the layout
// stays clean regardless of which fields a given record actually filled in.
const dynamicDetailFields = computed(() => {
